...
C. 模型調度守則（獨立檔案），至少涵蓋：
   ...
   - 顯式指定 model 與 effort：按實際可用型號寫，不憑印象。
   - 回報合約：subagent 只回結論與 檔案:行號，長產物落檔傳路徑。
   - 升降級路徑：小模型錯一次直接升級；中階同一子任務連錯兩次帶完整失敗軌跡升級；解出的 pattern 降回便宜模型批次套用；同一件事最多重試兩輪。
   - 驗證不自驗：檔案 read-back、程式碼實跑、高風險判斷加第二意見或多答案評審。
D. 判斷力外化（獨立檔案）：把只有你這個等級才擅長的判斷，寫成弱模型可執行的 rubric 與 checklist。至少涵蓋：
   - 何時該升級模型。
   - 何時算真的完成。
   - 何時該停下來問使用者。
   - 什麼訊號代表方向錯了該換路而非重試。
